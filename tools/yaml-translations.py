...
TRANSLATION_SUFIXES = ['.yaml', '.yml']

def translations_in_dir(path: Path, suffixes: set[str]=TRANSLATION_SUFIXES) -> set[Path]:
    return {p for p in path.glob("*") if p.suffix in suffixes}

def existing_languages(directory: Path) -> set[str]:
    return { f.stem for f in translations_in_dir(directory)}

def collect_json_files(paths):
    return collect_files(paths, [".json"])

def collect_yaml_files(paths):
    return collect_files(paths, TRANSLATION_SUFIXES)
    
def collect_files(paths, suffixes):
    files = set()
    for p in paths:
        files.update(translations_from(Path(p), suffixes))
    return sorted(files)
...
def lang_file(directory: Path, lang: str, fallback_extension: str = ".yaml") -> Path:
    ...
    Else use the fallback_extension, '.yaml' by default.
    """
...
def format_multiline_as_block(data, force: bool = False):
    ...
    If `force` is True, even single-line strings are converted.
    ...
    if isinstance(data, list):
        return [format_multiline_as_block(item, force) for item in data]

    if isinstance(data, dict):
        return {k: format_multiline_as_block(v, force) for k, v in data.items()}
...
def format_as_flow(data, force=False):
    """Convert strings to flow-style (normal str)."""
    if isinstance(data, ScalarString):
        if force or "\n" not in str(data):
            return str(data)
        return data
    ...
    if isinstance(data, list):
        return [format_as_flow(item, force=force) for item in data]

    if isinstance(data, dict):
        return {k: format_as_flow(v, force=force) for k, v in data.items()}
...
def sync(
    ...
    add_missing: bool = typer.Option(False, "--add-missing",
        help="Add missing ids from reference language with an empty string ready to fill",
    ),
    remove_extra: bool = typer.Option(False, "--remove-extra",
        help="Remove ids not in the reference language",
    ),
):
    """
    ...
    """
    files = collect_yaml_files(yamls)
    if not files:
...
def move(
    ...
    ids: list[str] = typer.Argument(..., help="IDs or namespaces to move"),
):
    """Move translation keys or namespaces from source to destination module."""
    src_files = collect_yaml_files([src_dir])
    if not src_files:
    ...
    for src_file in sorted(src_files):
        lang = src_file.stem
        dst_file = lang_file(dst_dir, lang, fallback_extension=src_file.suffix)
...
def andaluh(
    ...
    src_file = lang_file(path, SPANISH)
    if not src_file.exists():
        fail(f"Source language '{SPANISH}' not found in {path}")

    dst_file = lang_file(path, ANDALUSIAN)

    step(f"Source: {src_file}")
    step(f"Destination: {dst_file}")

    src_data = load_flat(src_file)
    dst_data = load_flat(dst_file)
    ...
    for key, value in src_data.items():
        if key in dst_data:
            if not overwrite:
                warn(f"{ANDALUSIAN}: '{key}' already exists, skipping")
                skipped += 1
                continue
            warn(f"{ANDALUSIAN}: '{key}' overwritten")

        dst_data[key] = translate_value(value)
        updated += 1

    if "language_code" in dst_data:
        dst_data["language_code"] = ANDALUSIAN
    if "language_name" in dst_data:
        dst_data["language_name"] = "Andalûh"

    dump_flat(dst_file, dst_data)

    success(f"{ANDALUSIAN}: {updated} keys updated, {skipped} skipped")

@app.command("to-json")
def to_json(
    paths: list[str] = typer.Argument(..., help="YAML files or directories"),
):
    """
    ...
    for file in files:
        ...
        success(f"{file.name} → {output.name}")

@app.command()
def from_json(
    paths: list[str] = typer.Argument(..., help="JSON files or directories"),
):
    """
    ...
    for file in files:
        ...
        data = format_multiline_as_block(data)
        output = file.with_suffix(".yaml")
...
@app.command()
def blockify(
    path: Path = typer.Argument(..., help="YAML file or directory"),
    ids: list[str] = typer.Argument(None, help="IDs or namespaces to process (all if omitted)"),
    force: bool = typer.Option(False, "--force", help="Convert also single-line strings"),
):
    """
    ...
    for file in files:
        ...
        dump_flat(file, data)
        success(f"{file.name}: {updated} values reformatted")

@app.command()
def flowify(
    path: Path = typer.Argument(..., help="YAML file or directory"),
    ids: list[str] = typer.Argument(None, help="IDs or namespaces to process (all if omitted)"),
    force: bool = typer.Option(False, "--force", help="Convert also multi-line"),
):
    """
